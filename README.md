...
As a fallback solution for cases where the defacing failed, an input transform can be produced by selecting landmarks pairs
...
7. Repeat steps 2-4 with the manual affine transformation
...
The code will loop through all defaced images and open the GIFs in MPV (this requires MPV to be installed and added to the
PATH). After assessing the quality of the defacing and closing MPV, the user is asked for an input string, 
...
1. Open 3D Slicer.
1. Drag the template image onto the 3D view anc confirm the "Add data into the scene" dialog. 
...
located in the same folder as the input image (but paths can be changed in the script): ![saving_the_transform.png](docs%2Fscreenshots%2Fsaving_the_transform.png)
